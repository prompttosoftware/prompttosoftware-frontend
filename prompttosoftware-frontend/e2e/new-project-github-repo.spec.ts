...
test.describe('New Project GitHub Repository Management UI', () => {
  ...
  test('should display "Add New Repository" and "Add Existing Repository" buttons', async ({ page }) => {
  ...
  test('should dynamically add new repository input fields', async ({ page }) => {
    ...
    await expect(page.locator('div[data-testid="repo-block-0"] [data-testid="repo-name-input-0"]')).toBeVisible();
    await expect(page.locator('div[data-testid="repo-block-0"] [data-testid="repo-org-input-0"]')).toBeVisible();
    await expect(page.locator('div[data-testid="repo-block-0"]')).toContainText('Private Repository');
    ...
    await expect(page.locator('div[data-testid="repo-block-2"] [data-testid="repo-url-input-2"]')).toBeVisible();
  });
  ...
  test('should enforce client-side validation for existing repository URL', async ({ page }) => {
    ...
    // Test valid URL
    await page.locator('input[placeholder="GitHub Repository URL"]').fill('https://github.com/test/repo');
    await page.getByRole('button', { name: 'Start' }).click();
  ...
  test('should capture form data correctly upon submission for new and existing repos', async ({ page }) => {
    ...
    await page.getByRole('button', { name: 'Add Existing Repository' }).click();
    await page.locator('input[placeholder="GitHub Repository URL"]').fill('https://github.com/existing/repo');
