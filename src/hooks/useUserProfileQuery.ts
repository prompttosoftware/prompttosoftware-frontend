...
import api, { fetchUserProfile } from '@/lib/api'; // Import fetchUserProfile
    ...
    useEffect(() => {
        if (isError) {
            if (axios.isAxiosError(error) && error.response?.status === 401) {
                ...
                logger.warn('401 Unauthorized response detected in useUserProfileQuery. Invalidating user profile query.');
