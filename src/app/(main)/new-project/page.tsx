...
export default function NewProjectPage() {
  const { register, handleSubmit, setError: setFormFieldError, formState: { errors } } = useForm<NewProjectRequest>();
  ...
  const onSubmit = async (data: NewProjectRequest) => {
    ...
    try {
      ...
      const response = await fetch('/api/projects', { // Endpoint should be correctly handled by MSW
        method: 'POST',
        headers: {
      ...
      try {
        ...
        parsedError = JSON.parse(rawError.message);
      } catch (parseError) {
        // If parsing fails, it's a generic error or a real network error
        parsedError = { status: 500, message: rawError.message || 'An unexpected error occurred.' };
      }
      ...
      if (parsedError.status === 400 && parsedError.errors) {
        ...
        setGlobalError({ message: parsedError.message || 'Please check the form for errors.', type: 'warning' });
      }
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="bg-white p-6 rounded-lg shadow-md">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          {/* Max Runtime Hours */}
          <div>
            <label htmlFor="maxRuntimeHours" className="block text-sm font-medium text-gray-700 mb-1">Max Runtime (Hours)</label>
            <input
              type="number"
              id="maxRuntimeHours"
          ...
          <div className="pt-4">
            ...
            >
              {isSubmitting ? 'Starting...' : 'Start'} {/* Change button text based on loading state */}
            </button>
          </div>
        </form>
      </div>
