...
export default function MainLayout({
  ...
  const navMarginClass = isNavExpanded
    ? 'ml-0 md:ml-64'
    : 'ml-0 md:ml-20';
  ...
  return (
    <div className="flex min-h-screen bg-gray-100">
      ...
      />
      {/* Header */}
      <header className={`fixed top-0 left-0 right-0 h-16 bg-white shadow-md flex items-center justify-between px-4 md:px-8 z-30 transition-all duration-300 ease-in-out ${isNavExpanded ? 'md:pl-64' : 'md:pl-20'}`}>
      ...
      {/* Main Content Area */}
      <main className={`flex-1 transition-all duration-300 ease-in-out ${navMarginClass} p-8 mt-16`}>
        {children}
      </main>
    </div>
  );
