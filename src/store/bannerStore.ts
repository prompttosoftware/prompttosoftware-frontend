...
const useBannerStore = create<BannerStore>((set, get) => ({
    ...
    initBanners: (initialBanners) => {
        ...
        try {
            ...
            const newBanners = initialBanners.filter(banner => !currentDismissedIds.includes(banner._id));
    ...
    addBanner: (banner) => {
        const newBanner: Banner = { ...banner, _id: `${Date.now()}-${Math.random().toString(36).substr(2, 9)}` };
        set((state) => {
            const updatedQueue = [...state.bannerQueue, newBanner].filter(b => !get().dismissedBannerIds.includes(b._id)); // Filter out dismissed
            if (!state.activeBanner && updatedQueue.length > 0) {
                const nextBanner = updatedQueue.shift();
                return { activeBanner: nextBanner, bannerQueue: updatedQueue };
            }
    ...
    setNextBanner: () => {
        set((state) => {
            ...
            const playableQueue = state.bannerQueue.filter(b => !state.dismissedBannerIds.includes(b._id));
    ...
    dismissCurrentBanner: () => {
        set((state) => {
            ...
            if (activeBanner) {
                const updatedDismissedIds = [...dismissedBannerIds, activeBanner._id];
                ...
                const nextBannerQueue = state.bannerQueue.filter(b => !updatedDismissedIds.includes(b._id));
